    ...
    PausableUpgradeable
{
    ...
    event CoverUsed(address indexed token, uint256 loss, uint256 covered, uint256 shortfall);
    event InsuranceFundExhausted(address indexed token, uint256 shortfall);
    ...
    function cover(address token, uint256 loss)
        external
        onlyRole(OPERATOR_ROLE)
        nonReentrant
        whenNotPaused
        returns (uint256 covered, uint256 shortfall)
    {
        ...
        // Effects
        balances[token] = balance - covered;
        ...
        emit CoverUsed(token, loss, covered, shortfall);

        if (shortfall > 0) {
            emit InsuranceFundExhausted(token, shortfall);
        }
    }
    ...
    /// @notice Get current fund balance for a token.
    /// @param token ERC20 token address.
